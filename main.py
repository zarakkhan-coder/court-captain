# CourtCaptain — votes, results, 4-player rule, weather, availability placeholder
...
LAT = float(os.environ.get("WALTON_LAT", "36.372"))   # change if you know exact coords
LON = float(os.environ.get("WALTON_LON", "-94.208"))
WALHEALTH_COOKIE = os.environ.get("WALHEALTH_COOKIE", "")  # optional if website needs login
...
def fetch_weather(day_name:str):
    """Open-Meteo (no key). Returns forecast dict for Saturday/Sunday."""
    try:
        ...
        return next((x for x in out if x["weekday"].lower().startswith(day_name[:3].lower())), None)
    except Exception:
        return None

def fetch_availability():
    """
    Placeholder for wholehealth.walmart.com court availability.
    If it requires auth, set WALHEALTH_COOKIE env var with your session cookie.
    Returns dict of {court_name: True/False}.
    """
    try:
        headers = {}
        if WALHEALTH_COOKIE:
            headers["Cookie"] = WALHEALTH_COOKIE
        # TODO: Replace this with the real endpoint/HTML parse when known.
        # For now: assume preferred courts are available.
        avail = {c: True for c in ALL_COURTS}
        return avail
    except Exception:
        return {c: True for c in ALL_COURTS}

# ---------- templates (simple two-pass, no inheritance) ----------
BASE = """
...
<div class="card">
  <form method="POST" action="{{ url_for('reset') }}" onsubmit="return confirm('You will be asked for a PIN to clear all votes. Continue?')">
...
<div class="card">
  ...
  <div class="table">
    ...
    {% for item in s.counts %}
      <div class="row"><div>{{ item.day }}</div><div>{{ item.court }}{% if item.court in preferred %} ★{% endif %}</div><div>{{ item.votes }}</div></div>
    {% else %}
...
<div class="card">
  ...
  <div class="table">
    ...
    {% for c in courts %}
      <div class="row"><div>{{ c }}</div><div>{{ "Available" if availability.get(c, True) else "Unavailable" }}</div><div>{% if c in preferred %}Preferred{% endif %}</div></div>
    {% endfor %}
  </div>
  <p class="note">If availability needs login, set WALHEALTH_COOKIE in Environment.</p>
</div>

<div class="glass"><p class="note">By Thursday, lock in the choice and book on the Walton Fitness Centre site.</p></div>
"""
...
def results():
    ...
    wx = None
    if s.get("top_choice") and is_outdoor(s["top_choice"]["court"]):
        wx = fetch_weather(s["top_choice"]["day"])
    return render_view(RESULTS, summary=s, preferred=PREFERRED_COURTS, courts=ALL_COURTS,
